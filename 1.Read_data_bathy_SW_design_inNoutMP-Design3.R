...
## Outside mp poly----
xcoord <- c(114.9244,114.9312,114.45724,114.4545,114.9244)
ycoord <- c(-33.9643,-34.0498,-34.0499,-33.9709,-33.9643)
...
writeOGR(poly.in.df, s.dir, "PolyInMP-d3", driver="ESRI Shapefile")
writeOGR(poly.out.df, s.dir, "PolyOutMP-d3", driver="ESRI Shapefile")
...
# save new bathy ----
writeRaster(b2, paste(s.dir, "SWBathy-InNOutMP-D3.tif", sep='/'), overwrite=T)
...
wp[v1, ] # 98 features left

# plot --
plot(b2)
plot(wp[v1, ], pch=20, col="black", add=T)
...
# save --
writeOGR( swbruvsu, dsn=d.dir, layer=paste( "DeansFiltered_utm", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
writeOGR( swbruvs, dsn=d.dir, layer=paste( "DeansFiltered_latlong", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
...
writeRaster(tpi, paste(s.dir, "SW-TPI-InNOutMP-D3.tif", sep='/'), overwrite=T)
...
writeRaster(slope, paste(s.dir, "SW-Slope-InNOutMP-D3.tif", sep='/'), overwrite =T)
...
saveRDS( SWDat, file="SWData_forInNOutMP-d3.RDS")

sw_rasters <- list()
sw_rasters$bathy <- b2
sw_rasters$slope <- slope
sw_rasters$tpi <- tpi
saveRDS( sw_rasters, file="SWRasters_forInNOutMP-d3.RDS")
saveRDS(zones, file="SWZones_forInNOutMP-d3.RDS")
...
plot(b2)
plot(zones$All2, add=T)
points(swbruvs) # Dean's points

# Points in or out marine park ----
Dpoints <- raster::extract(zones$All2, swbruvs, sp=T)
Dpoints

wpdf <- as.data.frame(swbruvs)
...
write.csv(Dpoints5, paste(d.dir, "DeansPoints_forinNOutMP-d3.csv", sep ='/'))
BRUVS <- SpatialPointsDataFrame( coords=Dpoints5[,c("Longitude","Latitude")], data=Dpoints5, 
                                 proj4string = CRS( proj4string( mp)))
saveRDS( Dpoints5, file="DeansPoints_forinNOutMP-d3.RDS")
