...
inclProbs <- raster(paste(d.dir, "inclProbs_InNOutMP-d2.tif", sep='/'))
plot(inclProbs)
...
zones <- readRDS( "SWZones_forInNOutMP-d2.RDS") # this one in different folder
Deans <- readRDS( "DeansPoints_forinNOutMP-d2.RDS")
swrast <- readRDS("SWRasters_forInNOutMP-d2.RDS")
...
straw.nums <- readRDS( "StrawmanNumbers_Zones-d2.RDS")
...
str(Deans3) # 40 Obs
Deans3$x <- Deans3$Longitude
Deans3$y <- Deans3$Latitude
head(Deans3)
...
fullSampled <- do.call( "rbind", fullSampled[1:40])
...
pdf(paste(w.dir, "InNOutMP", "InNOutMP-Clusters-d2.pdf", sep='/'), height=7, width=8)
plot(inclProbs, main = "Inclusion probabilities - Clusters - Design2")
...
pdf(paste(w.dir, "InNOutMP", "InNOutMP-Bathy-BRUVs-Clusters-d2.pdf", sep='/'), height=7, width=8)
plot(swrast$bathy, main ="Bathymetry - Clusters - D2", col = rev(brewer.pal(20, "RdYlBu")))
