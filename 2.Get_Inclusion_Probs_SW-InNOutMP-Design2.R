...
SWDat <- readRDS( "SWData_forInNOutMP-2.RDS")
sw_rasters <- readRDS("SWRasters_forInNOutMP-d2.RDS")
zones <- readRDS( "SWZones_forInNOutMP-d2.RDS")
...
straw.nums <- c(50, 24)  #numbers of drops in and out
straw.props <- straw.nums / sum( straw.nums) # 0.625 0.375
names( straw.nums) <- names( straw.props) <- c("In", "Out")
saveRDS( straw.nums, file="StrawmanNumbers_Zones-d2.RDS")
...
Bathy.targetNums <- rep( floor( 74/8), 2) # 8 8
...
props <- props / sum( props) # inside  0.736899  -  outside0.263101  
...
writeRaster(catB, file='Bathy_cuts_InNOutMP-d2.tif', overwrite=TRUE)
...
writeRaster( inclProbs, file='inclProbs_InNOutMP-d2.tif', overwrite=TRUE)
