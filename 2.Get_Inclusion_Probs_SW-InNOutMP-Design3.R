...
SWDat <- readRDS( "SWData_forInNOutMP-d3.RDS")
sw_rasters <- readRDS("SWRasters_forInNOutMP-d3.RDS")
zones <- readRDS( "SWZones_forInNOutMP-d3.RDS")
...
straw.nums <- c(40, 40)  #numbers of drops in and out
straw.props <- straw.nums / sum( straw.nums) # 0.625 0.375
names( straw.nums) <- names( straw.props) <- c("In", "Out")
saveRDS( straw.nums, file="StrawmanNumbers_Zones-d3.RDS")
...
Bathy.targetNums <- rep( floor( 80/8), 2) # 8 8
...
writeRaster(catB, file='Bathy_cuts_InNOutMP-d3.tif', overwrite=TRUE)
...
writeRaster( inclProbs, file='inclProbs_InNOutMP-d3.tif', overwrite=TRUE)
