...
## Outside mp poly----
xcoord <- c(114.9312,114.9312,114.4700,114.4672,114.9312)
ycoord <- c(-34.0111,-34.0498,-34.0484,-34.0165,-34.0111)
...
## Inside mp poly----
xcoord <- c(114.9314,114.9556,114.4824,114.4735,114.9314)
ycoord <- c(-34.0507,-34.1505,-34.1500,-34.0504,-34.0507)
xym <- cbind(xcoord, ycoord)
xym
...
# save new bathy ----
writeRaster(b2, paste(s.dir, "SWBathy-InNOutMP.tif", sep='/'))
...
writeRaster(tpi, paste(s.dir, "SW-TPI-InNOutMP.tif", sep='/'))
...
writeRaster(slope, paste(s.dir, "SW-Slope-InNOutMP.tif", sep='/'))
...
saveRDS( SWDat, file="SWData_forInNOutMP.RDS")

sw_rasters <- list()
sw_rasters$bathy <- b2
sw_rasters$slope <- slope
sw_rasters$tpi <- tpi
saveRDS( sw_rasters, file="SWRasters_forInNOutMP.RDS")
saveRDS(zones, file="SWZones_forInNOutMP.RDS")
...
#crop to those within domain ----

Dpoints5 <- Dpoints4[!is.na( Dpoints4$f),]
str(Dpoints5) # 53 points
Dpoints6 <- Dpoints5
coordinates(Dpoints6) <- ~Longitude+Latitude
points(Dpoints6, pch=20)


write.csv(Dpoints5, paste(d.dir, "DeansPints_forinNOutMP.csv", sep ='/'))
BRUVS <- SpatialPointsDataFrame( coords=Dpoints5[,c("Longitude","Latitude")], data=Dpoints5, 
                                 proj4string = CRS( proj4string( mp)))
saveRDS( Dpoints5, file="DeansPints_forinNOutMP.RDS")
