...
ip <- raster(paste(s.dir, "InclProbs-FishHWY-d5.tif", sep='/'))
plot(ip)

## read the spatial points created with MBH ----

p1 <- readOGR(paste(d.dir, "FHWY-BRUVS-d5.shp", sep='/')) # 240 features

p2 <- readOGR(paste(d.dir, "FHWY-BRUVS-d3.shp", sep='/')) # 240 features
...
## p1 ----
p1_matrix <- gWithinDistance(p1u, dist = 280, byid = TRUE)
diag(p1_matrix) <- NA
p1_matrix
...
# save --
writeOGR( swbruvsu, dsn=paste(d.dir, 'final', sep='/'), layer=paste( "FHWY-Bruvs-d5_utm", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
writeOGR( swbruvs, dsn=paste(d.dir, 'final', sep='/'), layer=paste( "FHWY-Bruvs-d5_latlong", Sys.Date(), sep="_"), driver="ESRI Shapefile", overwrite_layer=TRUE)
...
pdf(paste(w.dir, "Fish-HWY", "FishHWY-Bathy-Map-design5.pdf", sep='/'), height=7, width=8)
plot(b, col=viridis(100), main = "Recreational fishermen highway sampling design")
plot(p1[v1, ], pch=20, cex=1, col="black", add=T)
plot(p1[v1, ], pch=20, cex=0.6, col="white", add=T)
#plot(c7, pch=20, col="blue", add=T)
dev.off()
